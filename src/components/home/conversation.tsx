...
const Conversation = ({ conversation }: { conversation: any }) => {
	...
	const conversationName = conversation.groupName || (conversation.name == "Guest " ? conversation.email.split('@')[0] : conversation.name);
			...
			<div className={`flex gap-2 items-center p-3 hover:bg-chat-hover cursor-pointer 
				...
				<div className='w-full'>
					...
					<p className='text-[12px] mt-1 text-gray-500 flex items-center gap-1 '>
						...
						{lastMessageType === "image" && <> <ImageIcon size={16} /> <span className="text-xs">Photo</span></>}
